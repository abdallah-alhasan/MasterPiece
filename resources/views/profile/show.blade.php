				...
				<div class="tab-pane fade show active" id="tab-1" role="tabpanel">
					<div class="row">
						<div class="col-12" style="margin-bottom:40px;">
							<h3 class="section__title" >My Requests</h3>
						</div>
						<div class="col-12">
						...
						<div class="col-12 mb-5">
							<div class="paginator">
								<div class="paginator__counter">
									{{$requests->count() + 4 * ($requests->currentPage() - 1)}} from {{$requests->total()}}
								</div>

								<ul class="paginator__wrap">
									<li class="paginator__item paginator__item--prev">
										<a href={{'?page=' . ($requests->currentPage() - 1)}}>
									...
									@for ($i = 1; $i <= ceil($requests->total() / $requests->perPage()); $i++)
										<li class="paginator__item {{$i == $requests->currentPage() ? 'paginator__item--active' : ''}}"><a href=?page={{$i}}>{{$i}}</a></li>
									@endfor
					...
					<div class="row">
						<div class="col-12" style="margin-bottom:40px;">
							<h3 class="section__title" >My Trades</h3>
						</div>
						<div class="col-12">
							<div class="table-responsive table-responsive--border">
								<table class="profile__table">
						...
						<div class="col-12 mb-5">
							<div class="paginator">
								<div class="paginator__counter">
									{{$trades->count() + 4 * ($trades->currentPage() - 1)}} from {{$trades->total()}}
								</div>
								...
								<div class="row">
									<div class="col-12" style="margin-bottom:40px;">
										<h3 class="section__title" >Incoming Requests</h3>
									</div>
									<div class="col-12">
										<div class="table-responsive table-responsive--border">
											<table class="profile__table">
												...
												<tbody>
													@foreach ($incoming as $index => $request)
														<tr>
															<td><a href="/trade/{{$request->trade_id}}">{{$index + 1}}</a></td>
															<td>
																<div class="profile__img">
									...
									<div class="col-12 mb-5">
										<div class="paginator">
											<div class="paginator__counter">
												{{$incoming->count() + 4 * ($incoming->currentPage() - 1)}} from {{$incoming->total()}}
											</div>
			
											<ul class="paginator__wrap">
												<li class="paginator__item paginator__item--prev">
													<a href={{'?page=' . ($incoming->currentPage() - 1)}}>
														<svg xmlns='http://www.w3.org/2000/svg' width='512' height='512' viewBox='0 0 512 512'><polyline points='244 400 100 256 244 112' style='fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-width:48px'/><line x1='120' y1='256' x2='412' y2='256' style='fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-width:48px'/></svg>
													</a>
												</li>
												@for ($i = 1; $i <= ceil($incoming->total() / $incoming->perPage()); $i++)
													<li class="paginator__item {{$i == $incoming->currentPage() ? 'paginator__item--active' : ''}}"><a href=?page={{$i}}>{{$i}}</a></li>
												@endfor
												<li class="paginator__item paginator__item--next">
													<a href={{'?page=' . ($incoming->currentPage() + 1)}}>
														<svg xmlns='http://www.w3.org/2000/svg' width='512' height='512' viewBox='0 0 512 512'><polyline points='268 112 412 256 268 400' style='fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-width:48px'/><line x1='392' y1='256' x2='100' y2='256' style='fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-width:48px'/></svg>
